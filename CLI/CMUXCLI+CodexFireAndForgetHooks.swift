...
extension CMUXCLI {
    ...
    ///
    /// Layering contract (verified against codex-cli 0.146.0 and 0.153.4 by
    /// tests/test_codex_wrapper_hook_append.py): Codex discovers hooks per
    /// configuration layer and appends them from lowest to highest, so the
    /// user's `hooks.json` and `[hooks]` in `config.toml`, a trusted project's
    /// `.codex/hooks.json`, and profile layers all keep running ahead of these
    /// session-flag entries. A `-c hooks.<event>=` value only defines the
    /// session-flags layer; it never replaces a lower layer, and copying lower
    /// layers into it would make Codex discover and run every user handler
    /// twice. Each value therefore carries exactly one cmux group.
