...
final class SurfaceCatalog {
    ...
    /// The filtered overload is used by the cloud registry so a failed fleet read can
    /// still refresh local providers without touching cloud rows.
    ...
    /// Unfiltered spelling retained for callers that only need a full catalog refresh.
    func refreshAll() async {
        await refreshAll(where: { _ in true })
    }
