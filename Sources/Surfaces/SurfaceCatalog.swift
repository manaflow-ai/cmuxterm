...
final class SurfaceCatalog {
    ...
    /// Compatibility state for versioned cloud resource snapshots that predate the typed
    /// `CloudVMState` publication path. The catalog remains the sole owner; the state only
    /// retains the cursor and canonical workspace identities needed to fence legacy writers.
    @ObservationIgnored
    var cloudResourceCompatibility: [SurfaceMachineID: CloudResourceCompatibilityState] = [:]
    /// Catalog-owned optimistic workspace rename intents used by legacy callers while their
    /// provider obtains a read-your-write cursor receipt.
    @ObservationIgnored
