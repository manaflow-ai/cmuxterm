...
/// top-level value that is not an object, sanitizer failure, or a requested
/// value that cannot be decoded as its declared type) propagates to the caller,
/// who decides whether to fall back to defaults (reads) or refuse to mutate
/// (writes).
public enum JSONConfigStoreReadError: Error, Equatable, Sendable {
    ...
    /// The requested key exists, but its persisted value cannot be decoded as
    /// the key's declared type.
    case valueNotDecodable(key: String)
}
