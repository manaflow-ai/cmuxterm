...
/// The shared Swift fuzzy matcher: token scoring over word segments with
/// exact/prefix/contains/initialism/stitched-prefix and single-edit
/// fallbacks. Pure logic; scores are deterministic for a given
/// query/candidate pair.
///
/// Lives in `CmuxFoundation` because more than one domain ranks text against
/// a typed query - command-palette results and the sidebar workspace filter
/// both score through it, and neither may depend on the other.
///
...
public struct FuzzyMatcher {
    ...
    public struct PreparedToken: Equatable, Sendable {
        ...
        public init(_ normalizedText: String) {
            ...
            self.containsTokenBoundaryCharacter = characters.contains {
                FuzzyMatcher.tokenBoundaryChars.contains($0)
            }
    ...
    public struct PreparedCandidateText: Sendable {
        ...
        public init(normalizedText: String) {
            self.normalizedText = normalizedText
            self.characters = Array(normalizedText)
            self.wordSegments = FuzzyMatcher.wordSegments(characters)
            self.asciiMask = ASCIIScalarMask(normalizedText)
        }
    }
