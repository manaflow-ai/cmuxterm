...
public struct SSHRetryBackoffScriptBuilder: Sendable {
    ...
    public var signalHandlerBranches: String {
        "elif [ -n \"${\(backoffPIDVariable):-}\" ]; then builtin kill -TERM \"$\(backoffPIDVariable)\" >/dev/null 2>&1 || true; wait \"$\(backoffPIDVariable)\" 2>/dev/null || true; \(backoffPIDVariable)=; \(terminalInputRestoreLine) elif [ \"${\(backoffLaunchingVariable):-0}\" = 1 ]; then \(pendingSignalVariable)=\"$\(signalStatusVariable)\"; \(pendingSignalNameVariable)=\"$\(signalNameVariable)\"; return;"
    }
