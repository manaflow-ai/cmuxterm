...
struct SurfaceCatalogTests {
    ...
    @Test func `Cloud state replacement updates every workspace projection`() throws {
        let machine = SurfaceMachineID.cloud("vivid-newt")
        let catalog = SurfaceCatalog()
        let snapshot: [String: Any] = [
            ...
            "tabs": [
                ["id": "tab_main", "pane_id": "pane_main", "content_kind": "terminal", "content_id": "term_main"],
                ["id": "tab_other", "pane_id": "pane_other", "content_kind": "terminal", "content_id": "term_other"],
            ],
            "terminals": [
                ["id": "term_main", "tab_id": "tab_main", "tab_ids": ["tab_main"], "title": "shell", "lifecycle": "running"],
                ["id": "term_other", "tab_id": "tab_other", "tab_ids": ["tab_other"], "title": "other", "lifecycle": "running"],
            ],
            "browsers": [],
            "agents": [],
        ]
        ...
        #expect(catalog.snapshot.machines.first?.remoteWorkspaces?.map(\.name) == ["main", "other"])
        #expect(catalog.snapshot.resources(on: machine).count == 2)
        #expect(catalog.snapshot.resources(on: machine).allSatisfy { $0.remoteWorkspace != nil })

        let renamed = SurfaceRemoteWorkspace(id: "ws_main", name: "renamed", index: 0, focused: true)
        info.remoteWorkspaces = [renamed, SurfaceRemoteWorkspace(id: "ws_other", name: "other", index: 1, focused: false)]
        var renamedState = state
        renamedState.workspaces[0].name = "renamed"
        var renamedResources = resources
        for index in renamedResources.indices {
            if renamedResources[index].remoteWorkspace?.id == renamed.id {
                renamedResources[index].remoteWorkspace = renamed
                renamedResources[index].remoteViews = renamedResources[index].remoteViews?.map { view in
                    var updated = view
                    updated.workspace = renamed
                    return updated
                }
            }
        }
        catalog.replaceCloudState(renamedState, resources: renamedResources, info: info)
        #expect(catalog.snapshot.machines.first?.remoteWorkspaces?.first?.name == "renamed")
        #expect(catalog.snapshot.resources(on: machine).first { $0.id.key == "term_main" }?.remoteWorkspace?.name == "renamed")
    }
}
