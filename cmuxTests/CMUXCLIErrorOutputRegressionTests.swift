...
@Suite(.serialized) struct CMUXCLIErrorOutputRegressionTests: Sendable {
    ...
    @Test func testRejectedCurrentCaptureDoesNotCreateFlagOnlyMappedCodexLaunch() throws {
        let transcriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("cmux-codex-permissions-empty-\(UUID().uuidString).jsonl")
        let transcript = #"{"timestamp":"1970-01-01T00:01:30.000Z","type":"turn_context","payload":{"approval_policy":"never","sandbox_policy":{"type":"danger-full-access"}}}"#
        try Data((transcript + "\n").utf8).write(to: transcriptURL)
        defer { try? FileManager.default.removeItem(at: transcriptURL) }
        ...
        #expect(selected?.arguments.isEmpty == true)
    }
