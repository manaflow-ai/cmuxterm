...
| `vm terminal rename <machine> <terminal-id> <name> [--json]` | `vm.terminal_rename`: names the terminal's daemon tab view(s) through the same machine-scoped rename lane; every client shows the name in place of the PTY title, and the daemon persists it. Pass `""` to clear the custom label on every placement. A zero-view pool terminal has no tab to name. |
...
| `vm rename <id> <label>`, `vm rename <id> --clear` | Set or clear the machine's display-only label through `vm.rename`; the generated machine id remains its address. |
| `vm rm <id>` (aliases `destroy`, `delete`) | Permanently delete the machine and its data through `vm.destroy`. Confirm the id before running it. |
| `vm ssh <id>` | Open the provider SSH-backed workspace through the app's SSH attach path. This is a provider-dependent diagnostic/compatibility path; the normal shell path is `vm shell`. |
