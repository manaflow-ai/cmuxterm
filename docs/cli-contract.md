...
surface is still pending. The source-derived dispatch inventory is generated
by `scripts/generate-cli-rust-command-inventory.py` and checked by
`scripts/check-cli-rust-parity.py`; a changed Swift dispatch makes the check
fail until the inventory is regenerated. The required behavior contract is in
[`cli-rust-agent-system-plan.md`](cli-rust-agent-system-plan.md).

`cmux capabilities --json` is app-authoritative and sends
`system.capabilities` through the socket. `cmux capabilities --offline` is a
Rust migration catalog for development only; its entries are marked
experimental and do not prove production parity.
...
| `context` | Print the caller's non-secret workspace, surface, tab, socket, and authentication-source context as JSON. Rust migration builds may expose this before full command cutover. |
