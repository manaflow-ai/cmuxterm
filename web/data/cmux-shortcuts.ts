...
const paneWidthGrowthNote: LocalizedText = {
  ar: "كل ضغطة تزيد عرض اللوحة المركّزة بمقدار 20 بكسل عند أقرب تقسيم عرض؛ لا يتم حفظ حالة مؤقتة؛ لمساحات العمل المحلية ولوحات Dock فقط، بينما يتحكم tmux في تخطيط مرايا tmux البعيدة",
  bs: "svaki pritisak povećava širinu fokusiranog panela za 20 px na najbližoj podjeli širine; nema privremenog stanja; samo za lokalne radne prostore i Dock panele, dok tmux upravlja rasporedom udaljenih tmux ogledala",
  da: "hvert tryk øger det fokuserede panels bredde med 20 px ved den nærmeste breddeopdeling; ingen midlertidig tilstand gemmes; kun lokale arbejdsområder og Dock-paneler, mens tmux styrer layoutet i eksterne tmux-spejle",
  de: "jeder Tastendruck vergrößert den fokussierten Bereich an der nächsten Breitenaufteilung um 20 px; kein temporärer Zustand; nur lokale Arbeitsbereiche und Dock-Bereiche, während tmux das Layout entfernter tmux-Spiegel steuert",
  en: "each press grows the focused pane by 20px at its nearest width split; no temporary state is saved; local workspaces and Dock panes only, while tmux owns remote tmux mirror layout",
  es: "cada pulsación aumenta 20 px el ancho del panel enfocado en la división de ancho más cercana; no se guarda estado temporal; solo en espacios de trabajo locales y paneles del Dock, mientras tmux controla el diseño de los espejos tmux remotos",
  fr: "chaque pression élargit le panneau ciblé de 20 px à la division de largeur la plus proche ; aucun état temporaire n’est conservé ; uniquement dans les espaces de travail locaux et les panneaux du Dock, tandis que tmux contrôle la disposition des miroirs tmux distants",
  it: "ogni pressione aumenta di 20 px la larghezza del pannello attivo nella divisione più vicina; non viene salvato alcuno stato temporaneo; solo per spazi di lavoro locali e pannelli del Dock, mentre tmux controlla il layout dei mirror tmux remoti",
  ja: "押すたびに最も近い幅方向の分割でフォーカス中のペインが20px広がります。一時状態は保存しません。ローカルワークスペースとDockペインのみが対象で、リモートtmuxミラーのレイアウトはtmuxが管理します",
  km: "រាល់ការចុចបង្កើនទទឹងផ្ទាំងដែលផ្តោត 20px នៅការបែងចែកទទឹងជិតបំផុត; មិនរក្សាទុកស្ថានភាពបណ្ដោះអាសន្នទេ; សម្រាប់កន្លែងធ្វើការមូលដ្ឋាន និងផ្ទាំង Dock ប៉ុណ្ណោះ ខណៈ tmux គ្រប់គ្រងប្លង់កញ្ចក់ tmux ពីចម្ងាយ",
  ko: "누를 때마다 가장 가까운 너비 분할에서 포커스된 패널이 20px 늘어나며 임시 상태를 저장하지 않습니다. 로컬 작업 공간과 Dock 패널에만 적용되며 원격 tmux 미러 레이아웃은 tmux가 관리합니다",
  no: "hvert trykk øker bredden på det fokuserte panelet med 20 px ved nærmeste breddeinndeling; ingen midlertidig tilstand lagres; bare lokale arbeidsområder og Dock-paneler, mens tmux styrer oppsettet i eksterne tmux-speil",
  pl: "każde naciśnięcie zwiększa szerokość aktywnego panelu o 20 px w najbliższym podziale szerokości; stan tymczasowy nie jest zapisywany; tylko lokalne obszary robocze i panele Dock, a układem zdalnych kopii tmux zarządza tmux",
  "pt-BR": "cada pressionamento aumenta em 20 px a largura do painel em foco na divisão mais próxima; nenhum estado temporário é salvo; apenas em espaços de trabalho locais e painéis do Dock, enquanto o tmux controla o layout dos espelhos tmux remotos",
  ru: "каждое нажатие увеличивает ширину активной панели на 20 px в ближайшем разделении; временное состояние не сохраняется; только для локальных рабочих пространств и панелей Dock, а компоновкой удалённых зеркал tmux управляет tmux",
  th: "แต่ละครั้งจะเพิ่มความกว้างของบานหน้าต่างที่โฟกัส 20px ที่การแบ่งความกว้างที่ใกล้ที่สุด โดยไม่บันทึกสถานะชั่วคราว ใช้ได้เฉพาะพื้นที่ทำงานภายในเครื่องและบานหน้าต่าง Dock ส่วนเค้าโครงมิเรอร์ tmux ระยะไกลควบคุมโดย tmux",
  tr: "her basış odaktaki bölmeyi en yakın genişlik bölmesinde 20px büyütür; geçici durum kaydedilmez; yalnızca yerel çalışma alanları ve Dock bölmeleri içindir, uzak tmux yansılarının düzenini tmux yönetir",
  uk: "кожне натискання збільшує ширину активної панелі на 20 px у найближчому поділі; тимчасовий стан не зберігається; лише для локальних робочих просторів і панелей Dock, а компонуванням віддалених дзеркал tmux керує tmux",
  "zh-CN": "每次按键都会在最近的宽度分割处将聚焦面板增大 20px；不会保存临时状态；仅适用于本地工作区和 Dock 面板，远程 tmux 镜像的布局由 tmux 管理",
...
const paneHeightGrowthNote: LocalizedText = {
  ...
  da: "hvert tryk øger det fokuserede panels højde med 20 px ved den nærmeste højdeopdeling; ingen midlertidig tilstand gemmes; kun lokale arbejdsområder og Dock-paneler, mens tmux styrer layoutet i eksterne tmux-spejle",
  de: "jeder Tastendruck vergrößert den fokussierten Bereich an der nächsten Höhenaufteilung um 20 px; kein temporärer Zustand; nur lokale Arbeitsbereiche und Dock-Bereiche, während tmux das Layout entfernter tmux-Spiegel steuert",
  en: "each press grows the focused pane by 20px at its nearest height split; no temporary state is saved; local workspaces and Dock panes only, while tmux owns remote tmux mirror layout",
  es: "cada pulsación aumenta 20 px la altura del panel enfocado en la división de altura más cercana; no se guarda estado temporal; solo en espacios de trabajo locales y paneles del Dock, mientras tmux controla el diseño de los espejos tmux remotos",
  fr: "chaque pression agrandit le panneau ciblé de 20 px à la division de hauteur la plus proche ; aucun état temporaire n’est conservé ; uniquement dans les espaces de travail locaux et les panneaux du Dock, tandis que tmux contrôle la disposition des miroirs tmux distants",
  it: "ogni pressione aumenta di 20 px l’altezza del pannello attivo nella divisione più vicina; non viene salvato alcuno stato temporaneo; solo per spazi di lavoro locali e pannelli del Dock, mentre tmux controlla il layout dei mirror tmux remoti",
  ja: "押すたびに最も近い高さ方向の分割でフォーカス中のペインが20px高くなります。一時状態は保存しません。ローカルワークスペースとDockペインのみが対象で、リモートtmuxミラーのレイアウトはtmuxが管理します",
  km: "រាល់ការចុចបង្កើនកម្ពស់ផ្ទាំងដែលផ្តោត 20px នៅការបែងចែកកម្ពស់ជិតបំផុត; មិនរក្សាទុកស្ថានភាពបណ្ដោះអាសន្នទេ; សម្រាប់កន្លែងធ្វើការមូលដ្ឋាន និងផ្ទាំង Dock ប៉ុណ្ណោះ ខណៈ tmux គ្រប់គ្រងប្លង់កញ្ចក់ tmux ពីចម្ងាយ",
  ko: "누를 때마다 가장 가까운 높이 분할에서 포커스된 패널이 20px 늘어나며 임시 상태를 저장하지 않습니다. 로컬 작업 공간과 Dock 패널에만 적용되며 원격 tmux 미러 레이아웃은 tmux가 관리합니다",
  no: "hvert trykk øker høyden på det fokuserte panelet med 20 px ved nærmeste høydeinndeling; ingen midlertidig tilstand lagres; bare lokale arbeidsområder og Dock-paneler, mens tmux styrer oppsettet i eksterne tmux-speil",
  pl: "każde naciśnięcie zwiększa wysokość aktywnego panelu o 20 px w najbliższym podziale wysokości; stan tymczasowy nie jest zapisywany; tylko lokalne obszary robocze i panele Dock, a układem zdalnych kopii tmux zarządza tmux",
  "pt-BR": "cada pressionamento aumenta em 20 px a altura do painel em foco na divisão mais próxima; nenhum estado temporário é salvo; apenas em espaços de trabalho locais e painéis do Dock, enquanto o tmux controla o layout dos espelhos tmux remotos",
  ru: "каждое нажатие увеличивает высоту активной панели на 20 px в ближайшем разделении; временное состояние не сохраняется; только для локальных рабочих пространств и панелей Dock, а компоновкой удалённых зеркал tmux управляет tmux",
  th: "แต่ละครั้งจะเพิ่มความสูงของบานหน้าต่างที่โฟกัส 20px ที่การแบ่งความสูงที่ใกล้ที่สุด โดยไม่บันทึกสถานะชั่วคราว ใช้ได้เฉพาะพื้นที่ทำงานภายในเครื่องและบานหน้าต่าง Dock ส่วนเค้าโครงมิเรอร์ tmux ระยะไกลควบคุมโดย tmux",
  tr: "her basış odaktaki bölmeyi en yakın yükseklik bölmesinde 20px büyütür; geçici durum kaydedilmez; yalnızca yerel çalışma alanları ve Dock bölmeleri içindir, uzak tmux yansılarının düzenini tmux yönetir",
