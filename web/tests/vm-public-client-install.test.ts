...
import { CMUX_TUI_BINARY_PATH, cmuxTuiInstallCommand, shellQuote } from "../services/vms/drivers/cmuxTuiDaemon";

for (const prefix of ["cmux-public-client-", "cmux-public-client-'$; -"]) {
  test(`the public client works without private-home access (temporary path: ${prefix})`, () => {
    ...
    try {
      ...
      const command = cmuxTuiInstallCommand({ url: "https://example.invalid/must-not-download", sha256, commit: "fixture", builtAt: null })
        .replaceAll(shellQuote(CMUX_TUI_BINARY_PATH), shellQuote(source))
        .replaceAll(shellQuote(`${CMUX_TUI_BINARY_PATH}.tmp`), shellQuote(`${source}.tmp`))
        .replaceAll(shellQuote("/root/.cmux/bin"), shellQuote(join(privateHome, ".cmux/bin")))
        .replaceAll(shellQuote("/usr/local/bin/cmux-tui"), shellQuote(commandPath))
        .replaceAll(shellQuote("/usr/local/bin/cmux-tui.tmp.XXXXXX"), shellQuote(`${commandPath}.tmp.XXXXXX`));
